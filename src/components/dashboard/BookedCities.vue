...
const { bookedCities } = storeToRefs(analyticStore)
...
const option = computed(() => ({
  ...
  xAxis: {
    type: 'category',
    data: bookedCities.value.map(city => city.destination)
  },
  yAxis: {
    name: 'Count',
    type: 'value'
  ...
  series: [
    {
      name: 'travelers',
      data: bookedCities.value.map(city => city.analytics.travelers.score),
      type: 'bar',
    ...
    {
      name: 'flights',
      data: bookedCities.value.map(city => city.analytics.flights.score),
      type: 'bar',
